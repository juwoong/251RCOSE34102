...
GanttChart* rr_scheduling(ProcessQueue *queue, int *io_time, int io_time_length, int time_unit) {
    ...
    for(int i=0; i < max_time; i++) {
        ...
        if (current.pid == 0) {  // current is idle
            if (next_process.pid > 0) {
                ...
                if (current.pid > 0) {  // idle to running
                    if (current_start_time < i) add_gantt_item(gantt_chart, 0, current_start_time, i, 2);
        ...
        if (current_burst_time >= time_unit || current.cpu_burst_time - current_burst_time <= 0) {
            ...
            if (current.cpu_burst_time - current_burst_time > 0) {
                current.cpu_burst_time -= current_burst_time;
